...
let HotkeyListBox = class HotkeyListBox extends Gtk.TreeView {
    _init() {
        ...
        this.settings
            .list_keys()
            .map((key) => {
            const [_, accelKey, mods] = Gtk.accelerator_parse(this.settings.get_strv(key)[0]);
            let accelName;
            if (accelKey == 0) {
                accelName = 'Disabled';
            }
            ...
            log(modelEntry.key, modelEntry.summary, modelEntry.mods, modelEntry.accelKey);
            model.set(row, [0, 1, 2, 3], [
                modelEntry.key,
                modelEntry.summary,
                modelEntry.mods,
                modelEntry.accelKey,
    ...
    addSetting(key, type, customWidget) {
        ...
        switch (type) {
            ...
            case WidgetType.INT:
                ...
                this.settings.bind(key, widget.get_adjustment(), 'value', Gio.SettingsBindFlags.DEFAULT);
                break;
